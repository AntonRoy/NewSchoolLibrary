        ...
        try:
            ...
            if select == 'По ученику':
                ...
                if len(puple) > 1:
                    ...
                    books = executes_for_web.Search_Of_Student(puple[0])
                    return render_template('found.html', stname=name + ' ' + surname, klass=grade, arrays=books)
                else:
                    uch = 140
                    error = "Ничего не найдено"
                    return render_template('found.html', stname=name + ' ' + surname, klass=grade, arrays=None,
                                           error=error)
                return render_template('newmain.html', stname=name + ' ' + surname, klass=grade, arrays=puple, uch=uch,
                                       a_book=a_book, a_stud=a_stud, div_book=div_book, div_stud=div_stud, error=None)
